...
function x=dengueODE11_Modified(x,nclus, Temp, q, Nh,uh,kv,kh,gama,alpha,pv,ph,tau,k0, ep, Tp, Tl, Ti, Te,Pre01,Pre02,Pre03, mu1, mu2, mu3, mu4, mu5,mu6,nenv)
% definition of dependent variables
%tspan=[0 1];
tspan=getappdata(0,'TSPAN');
[t,y3]=ode23tb(@(t,x) laODE(t,x,nclus, Temp, q, Nh,uh,kv,kh,gama,alpha,pv,ph,tau, k0, ep, Tp, Tl, Ti, Te,Pre01,Pre02,Pre03, mu1, mu2, mu3, mu4, mu5,mu6,nenv),tspan,x);
x=y3(end,:)';
end

function vectorode=laODE(tt,x,nclus, temp, q,Nh,uh,kv,kh,gama,alpha,pv,ph,tau, k0, ep, Tp, Tl, Ti, Te,Pre01,Pre02,Pre03, mu1, mu2, mu3, mu4, mu5,mu6,nenv)
...
betah(i)=alpha(i)*ph(i)*(Sva(i)+Iv(i)+Ev(i))/div(i);    %Equation (10), (11)
end
...
for i=1:nclus %columna
   for j=1:nclus %fila
       ...
       sumahost(i)=q(i,j)*Iv(j)*betah(j)/(Sva(i)+Iv(i)+Ev(i))+sumahost(i); %suma sobre las columnas para S y E vector
   end
end
